...
export default function buildResolversFromModel(InModel: any, name?: String) {
    ...
    Object.keys(resolvers).forEach((k) => {
      resolvers[k] = resolvers[k].wrapResolve((next) => async (rp) => {
        ...
        rp.beforeRecordMutate = async function (doc, rp) {
          ...
          if (!rp.context.token) throw new Error('Forbidden!');
          return doc;
        };
  ...
  const Queries = {
    ...
    [`${name}Many`]: ModelTC.mongooseResolvers.findMany({ filter: { operators: true } }),
